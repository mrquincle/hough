...
struct DecPoint: Point2D {
	...
	// index in an array (if needs to be preserved)
	int index;

	// can be used for labels (part of line X)
	int cluster_index;
...
static const std::string SegmentationDescription[] = { "all points", "longest line", "distribution sensitive", 
	"glue points", "number of segmentation types" };
 ...
 */
class DetectLineModuleExt {
public:
	enum DataItem { DI_LINE, DI_SQUARE, DI_PENTAGON, DI_BOOKS, DI_POINT_LINES, NUMBER_OF_DATA_ITEMS };
	...
	//! Load a vector of points from a preprocessed image
	void loadImage(std::string file, pointcloud & cloud);

	void loadPoints(std::string ifilename, pointcloud & cloud);

	void writeImage(std::string ifile, std::string ofile);

	//! Write assignments
	void writeAssignments(std::string ofilename);
